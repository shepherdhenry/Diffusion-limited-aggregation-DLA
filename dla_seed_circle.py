...
This flie put seeds as a circle
...
# Place the initial seed in a circle
center = grid_size // 2
...
for n in range(len(coordsToCheck[0])):
    grid[int(grid_size/2-radiusCircle+coords[n,0]),int(grid_size/2-radiusCircle+coords[n,1])] = 1
##############################################
...
plt.title('DLA: Seed Circle')
plt.show()

np.save(f"grid_seed_circle_{6}.npy", grid)
