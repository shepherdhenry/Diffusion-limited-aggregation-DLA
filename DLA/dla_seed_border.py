...
This flie put seed on the border
...
def dla():
	...
	while( particlecount < num_particles ):
		...
		x = center + int((Rmin-5) * np.sin(angle))
		y = center + int((Rmin-5) * np.sin(angle))
		...
		while FLAG:
			...
			if ((x-center)*(x-center)+(y-center)*(y-center)>=(Rmin+2)*(Rmin+2)):
		...
		if(Notkill):
			if abs(x - center) < Rmin or abs(y - center) < Rmin:
				Rmin = min(abs(x - center), abs(y - center))
